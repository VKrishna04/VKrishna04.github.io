...
const useProjectsData = () => {
  ...
  const getCounterValue = (repoName, counterDataMap, config) => {
    if (!config.counterAPI?.enabled) return null;

    const { customMappings, autoGenerate } =
      config.counterAPI.projectMapping || {};

    // Check custom mappings first
    if (customMappings && customMappings[repoName]) {
      const mappedName = customMappings[repoName];
      return counterDataMap[mappedName] || null;
    }

    // Auto-generate mapping if enabled
    if (autoGenerate) {
      return counterDataMap[repoName] || null;
    }
  ...
  const fetchRepositories = async () => {
    ...
    try {
      ...
      const counterDataMap = await fetchCounterData(config);
      ...
      const reposWithLanguages = await Promise.all(
        filteredRepos.map(async (repo) => {
          ...
          return {
            ...
            counterValue: getCounterValue(repo.name, counterDataMap, config),
          };
        })
      );
